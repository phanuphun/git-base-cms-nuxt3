      ...
      <header class="w-full px-4 py-3 flex flex-wrap justify-between items-center shadow-md shadow-gray-200">
         ...
         <div class="w-auto flex flex-row gap-3">
          ...
          <input
            ...
            class="w-auto h-full border-2 border-gray-300 active:border-gray-600 rounded-md px-2"
            v-model="search"
            type="text"
            placeholder="Search" >
        </div> -->
            <NuxtLink :to="gitLink" target="_blank">
               <div class="cursor-pointer duration-200">
                  <img class="w-[30px] h-[30px] hover:scale-[1.05] duration-200" src="/github-logo.png"
                     alt="github-logo">
               </div>
            </NuxtLink>
         </div>
      ...
      <div class="w-1/4 h-auto border-l-2 border-gray-200 pt-8 px-2">
         ...
         <div class="">
            <ContentList v-if="search !== ''" :path="`/articles`" fields="title,path" :query="searchParams">
               ...
               <template #not-found>
                  <p class="text-gray-500 px-2 mt-2">ไม่พบบทความที่ตรงกับคำค้นหา</p>
               </template>
            </ContentList>
         </div>
      </div>
